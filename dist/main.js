...
async function githubAuth() {
    ...
    const deviceCodeRes = await axios.post('https://github.com/login/device/code', new URLSearchParams({
        client_id: CLIENT_ID,
    ...
    while (!authenticated && tries < maxTries) {
        ...
        tries++;
        try {
            const tokenRes = await axios.post('https://github.com/login/oauth/access_token', new URLSearchParams({
                client_id: CLIENT_ID,
                device_code,
